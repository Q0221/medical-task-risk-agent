...
- [x] 新增 `backend/Dockerfile` 与 `docker-entrypoint.sh`（等待 MySQL、迁移、启动 uvicorn）
- [x] 新增 `task_risk_vue/Dockerfile` 与 `nginx.conf`（静态站 + `/api/v1` 反代）
- [x] 新增 `docker-compose.prod.yml`：mysql/redis 内网、web 暴露 80、数据卷与 healthcheck
- [x] 新增 `.env.prod.example`，列出 AUTH/LLM/CORS 等生产必改项
